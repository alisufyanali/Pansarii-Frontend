...
export default function BeautyCornerCategoryPage() {
  const router = useRouter();
  const categoryName = "Beauty Corner";
      ...
      <div className="bg-gradient-to-r from-pink-50 to-rose-50 border-b border-pink-100">
        <div className="max-w-[1920px] mx-auto px-[4%] py-6 sm:py-10">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-pink-100 rounded-2xl flex items-center justify-center">
              {/* <FaPaintBrush className="w-6 h-6 sm:w-8 sm:h-8 text-pink-600" /> */}
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl 2xl:text-4xl font-bold text-gray-900 mb-1">Beauty Corner</h1>
              <p className="text-sm sm:text-base text-gray-600">Natural beauty and skincare products</p>
              <div className="flex items-center gap-3 mt-2">
                <span className="px-3 py-1 bg-pink-100 text-pink-700 rounded-full text-sm font-medium">
                  {productCount} Products
                </span>
                <span className="text-sm text-gray-500">Skincare • Makeup • Natural</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border-b border-gray-200 mt-4">
        <div className="max-w-[1920px] mx-auto px-[4%] py-3">
          <div className="flex items-center gap-2 text-sm">
            <button onClick={() => router.push('/category')} className="text-gray-500 hover:text-pink-600">Categories</button>
            <span className="text-gray-400">/</span>
            <span className="text-pink-700 font-medium">Beauty Corner</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1920px] mx-auto px-[4%] py-6 sm:py-8">
        ...
        {isLoading ? <GridSkeleton /> : filteredProducts.length > 0 ? (
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 sm:gap-6">
                    ...
                    <div className="flex-1 flex flex-col justify-between">
                      <div>
                        ...
                        {product.description && <p className="text-xs sm:text-sm text-pink-700 mb-3 line-clamp-2">{product.description}</p>}
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-gray-100">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {product.oldPrice && <span className="px-2 py-0.5 bg-pink-100 text-pink-700 rounded-full text-xs font-semibold">Save {Math.round(((product.oldPrice - product.price) / product.oldPrice) * 100)}%</span>}
                        </div>
                        <button className="px-6 py-2.5 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition font-semibold flex items-center justify-center gap-2 text-sm w-full sm:w-auto"
          ...
          <div className="text-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">No beauty products found</h3>
            <p className="text-gray-500 mb-6">{filters.searchQuery ? `No results for "${filters.searchQuery}"` : 'Try adjusting your filters'}</p>
            <button onClick={clearFilters} className="px-6 py-3 bg-pink-600 text-white rounded-lg hover:bg-pink-700 transition font-medium">Clear Filters</button>
          </div>
        )}
          ...
          <div className="mt-8 sm:mt-10 flex items-center justify-center gap-2">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
              <button key={page} onClick={() => { setCurrentPage(page); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                className={`w-10 h-10 flex items-center justify-center rounded-lg border text-sm font-medium ${currentPage === page ? 'bg-pink-600 text-white border-pink-600' : 'border-gray-300 hover:bg-gray-50'}`}>
                {page}
              </button>
            ))}
