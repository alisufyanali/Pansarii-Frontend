...
// ── Toast portal: renders ToastContainer at document.body level,
//    completely outside the modal's stacking context, with z-[99999]
//    so it always sits above the modal (z-[9999]).
...
export default function ProductDetailsModal({ product, onClose }: ProductDetailsModalProps) {
  ...
  return (
    <>
      {/* Toast rendered at document.body, above everything */}
      <ToastPortal />
                  ...
                  {[product.img, ...product.additionalImages].map((image, index) => (
                    <button
                      ...
                      className={`flex-shrink-0 w-12 h-12 sm:w-16 sm:h-16 rounded overflow-hidden border-2 transition-all ${
                        selectedImage === image ? 'border-green-600 shadow-md' : 'border-gray-200 hover:border-gray-400'
                      }`}
                    >
            ...
            <div className="lg:w-3/5 p-4 sm:p-5 lg:p-6">
              <div className="space-y-3 sm:space-y-4">
                <div>
                  <div className="flex items-start justify-between">
                    <div className="flex-1 pr-2">
                      <h1 className="text-lg sm:text-xl lg:text-2xl font-bold text-gray-900 leading-tight">{product.nameEn}</h1>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-2xl sm:text-3xl lg:text-4xl font-bold text-gray-900">PKR {product.price.toLocaleString()}</span>
                  {product.oldPrice && (
                    <span className="text-gray-500 line-through text-sm sm:text-base">PKR {product.oldPrice.toLocaleString()}</span>
                  )}
                </div>
                ...
                {product.benefits && product.benefits.length > 0 && (
                  <div className="flex flex-wrap gap-2">
                    {product.benefits.map((benefit, index) => (
                      <span key={index} className="px-2 sm:px-3 py-1 bg-green-50 text-green-700 rounded-full text-xs sm:text-sm font-medium">
                        {benefit}
                      </span>
                    ))}
                  </div>
                      ...
                      {product.sizes.map((size) => (
                        <button
                          ...
                          className={`px-3 sm:px-4 py-2 rounded-lg border-2 text-sm sm:text-base font-medium transition-all ${
                            selectedSize === size ? 'bg-green-600 text-white border-green-600 shadow-md' : 'border-gray-300 hover:border-green-600 hover:bg-gray-50'
                          }`}
                        >
                          {size}
                        </button>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      {product.features.slice(0, 6).map((feature, index) => {
                        ...
                        const hasCheck = typeof feature === 'string' ? feature.includes('✓') : feature.text?.includes('✓') || feature.hasCheck;
                        return (
                          <div key={index} className="flex items-center gap-2 p-2 sm:p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                            <div className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center flex-shrink-0 text-xs sm:text-sm ${hasCheck ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-600'}`}>
                              {hasCheck ? '✓' : '○'}
                            </div>
                            <span className="text-gray-700 text-xs sm:text-sm">{featureText}</span>
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
                      <div className="flex items-center border-2 border-gray-300 rounded-lg w-fit">
                        <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-600 hover:bg-gray-100 font-bold transition-colors">
                        ...
                        <button onClick={() => setQuantity(q => q + 1)} className="px-3 sm:px-4 py-2 sm:py-2.5 text-gray-600 hover:bg-gray-100 font-bold transition-colors">
                          <FaPlus className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                        </button>
                      </div>
                      <div>
                        <div className="text-gray-600 text-xs sm:text-sm">Subtotal:</div>
                        <div className="font-bold text-lg sm:text-xl lg:text-2xl text-gray-900">PKR {(product.price * quantity).toLocaleString()}</div>
                      </div>
                    </div>
                  </div>
