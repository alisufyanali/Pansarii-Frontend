...
export default function HerbCategoryPage() {
  const router = useRouter();
  const categoryName = "Herb";
        ...
        <div className="max-w-[1920px] mx-auto px-[4%] py-6 sm:py-10">
          <div className="flex items-start gap-4">
            <div className="w-12 h-12 sm:w-16 sm:h-16 bg-green-100 rounded-2xl flex items-center justify-center">
              <FaLeaf className="w-6 h-6 sm:w-8 sm:h-8 text-green-600" />
            </div>
            <div>
              <h1 className="text-2xl sm:text-3xl 2xl:text-4xl font-bold text-gray-900 mb-1">Herbs</h1>
              <p className="text-sm sm:text-base text-gray-600">Natural herbs and botanical products for wellness</p>
              <div className="flex items-center gap-3 mt-2">
                <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">
                  {productCount} Products
                </span>
                <span className="text-sm text-gray-500">Traditional • Natural • Organic</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white border-b border-gray-200 mt-4">
        <div className="max-w-[1920px] mx-auto px-[4%] py-3">
          <div className="flex items-center gap-2 text-sm">
            <button onClick={() => router.push('/category')} className="text-gray-500 hover:text-green-600">Categories</button>
            <span className="text-gray-400">/</span>
            <span className="text-green-700 font-medium">Herbs</span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1920px] mx-auto px-[4%] py-6 sm:py-8">
        ...
        {isLoading ? <GridSkeleton /> : filteredProducts.length > 0 ? (
          ...
          <div className="text-center py-16">
            ...
            <button onClick={clearFilters} className="px-6 py-3 bg-[#197B33] text-white rounded-lg hover:bg-[#156529] transition font-medium">Clear Filters</button>
          </div>
        )}
