...
export default function MobileFeaturedProducts() {
  ...
  const [products, setProducts] = useState<Product[]>(bestSellers.slice(0, 10) as Product[]);
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    getFeaturedProducts()
      .then(data => {
        setProducts((data.length > 0 ? data : bestSellers as Product[]).slice(0, 10));
      })
      .catch(() => {
        setProducts(bestSellers.slice(0, 10) as Product[]);
      })
    ...
    <section className="py-4">
      ...
      {isLoading && (
        <div className="flex gap-3 pl-4 pr-8">
          {[...Array(3)].map((_, i) => (
            <div key={i} className="feat-card flex-shrink-0 bg-gray-100 rounded-2xl animate-pulse h-24" style={{ width: '65vw' }} />
          ))}
        </div>
      )}
        ...
        >
          {products.map((product, index) => (
            <div
              ...
              style={{ width: '65vw' }}
              onClick={() => router.push(`/${product.slug ?? product.nameEn.toLowerCase().replace(/\s+/g, '-')}`)}
            >
